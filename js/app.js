...
function switchTab(tab) {
  document.querySelectorAll('.screen').forEach(s => { s.classList.add('hidden'); s.classList.remove('screen-active'); });
...
function openCarteirinha() { document.getElementById('modalCarteirinha').classList.remove('hidden'); }
function closeCarteirinha() { document.getElementById('modalCarteirinha').classList.add('hidden'); }

let toastTimer;
function showToast(msg) {
  const toast = document.getElementById('toast');
  const inner = toast.querySelector('.toast-run');
  document.getElementById('toastMsg').textContent = msg;
  toast.classList.remove('hidden'); toast.classList.add('flex');
  inner.classList.remove('toast-run'); void inner.offsetWidth; inner.classList.add('toast-run');
  clearTimeout(toastTimer);
  toastTimer = setTimeout(() => { toast.classList.add('hidden'); toast.classList.remove('flex'); }, 3400);
}
...
function toggleAcc(btn) {
  ...
  document.querySelectorAll('.accordion-body').forEach(b => { b.style.maxHeight = '0px'; const c = b.previousElementSibling.querySelector('.acc-chev'); if(c) c.style.transform = 'rotate(0deg)'; });
  if(!open) { body.style.maxHeight = body.scrollHeight + 'px'; chev.style.transform = 'rotate(180deg)'; }
}

document.querySelectorAll('.filter-chip').forEach(chip => {
  chip.addEventListener('click', () => {
    const f = chip.dataset.filter;
    document.querySelectorAll('.filter-chip').forEach(c => { c.className = 'filter-chip rounded-full bg-white px-4 py-2 text-[13px] font-semibold text-ink-70 ring-1 ring-edge'; });
    chip.className = 'filter-chip rounded-full bg-roxo px-4 py-2 text-[13px] font-bold text-white';
    document.querySelectorAll('.policy-card').forEach(card => { card.style.display = (f === 'todas' || card.dataset.cat === f) ? '' : 'none'; });
  });
});
...
const svg = (k, sz = 20) => `<svg width="${sz}" height="${sz}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">${IC[k]}</svg>`;
...
const TAB = {
  ...
  residencial: { taxaAno: { casa: .0024, apartamento: .0017 },
    ...
    { seg: 'Meridiano', nome: 'Blindado', fator: 1.55, cob: ['Tudo do Completo', 'Danos por água', 'Equipamentos portáteis', 'Assistência pet'] }
    ] },
  condominio: { taxaAno: .0012, tipoFator: { residencial: 1, comercial: 1.25, misto: 1.12 },
    ...
    { seg: 'Vitalis', nome: 'Completo', fator: 1.15, cob: ['Tudo da Obrigatória', 'Danos elétricos', 'Vidros e portões', 'Elevadores'] },
    { seg: 'Meridiano', nome: 'Master', fator: 1.5, cob: ['Tudo do Completo', 'RC ampliada', 'Roubo de bens comuns', 'Assistência predial 24h'] }
    ] },
  empresarial: { taxaAno: { comercio: .0035, servicos: .0028, escritorio: .0022, industria: .0052 },
    ...
    { seg: 'Vitalis', nome: 'Completo', fator: 1.2, cob: ['Tudo do Essencial', 'RC operações', 'Danos elétricos', 'Vidros e letreiros'] },
    { seg: 'Meridiano', nome: 'Master', fator: 1.55, cob: ['Tudo do Completo', 'Lucros cessantes', 'Fidelidade de funcionários', 'Cyber básica'] }
    ] },
  ...
  dando: { taxaLimite: { '1000000': .012, '2000000': .010, '5000000': .008, '10000000': .007 },
    ...
    { seg: 'Meridiano', nome: 'Global', fator: 1.55, cob: ['Tudo da Ampla', 'Cobertura internacional', 'Responsabilidade fiscal e trabalhista'] }
    ] },
...
const P = {
  vida: { t: 'Vida e Afastamento', full: 'Seguro Vida e Afastamento (DIT)', imediato: true, tag: 'Apólice na hora', cota: cotaVida, req: ['idade'] },
  saude: { t: 'Saúde', full: 'Seguro Saúde', imediato: false, tag: 'Sujeito a análise', cota: cotaSaude, req: ['idade', 'cidade'] },
  residencial: { t: 'Residencial', full: 'Seguro Residencial', imediato: true, tag: 'Emissão rápida', cota: cotaResidencial, req: ['cep'] },
  condominio: { t: 'Condomínio', full: 'Seguro Condomínio', imediato: false, tag: 'Sujeito a análise', cota: cotaCondominio, req: ['unidades', 'cep'] },
  empresarial: { t: 'Empresarial', full: 'Seguro Empresarial', imediato: false, tag: 'Sujeito a análise', cota: cotaEmpresarial, req: ['cep'] },
  rc: { t: 'Resp. Civil', full: 'Seguro de Responsabilidade Civil (RC)', imediato: false, tag: 'Sujeito a análise', cota: cotaRc, req: ['atividade'] },
  garantia: { t: 'Garantia', full: 'Seguro Garantia', imediato: false, tag: 'Análise de crédito', cota: cotaGarantia, req: ['valorGarantido'] },
  dando: { t: 'D&O', full: 'Seguro D&O', imediato: false, tag: 'Sob subscrição', cota: cotaDando, req: [] },
  consorcio: { t: 'Consórcio', full: 'Consórcio', imediato: false, tag: 'Simulação', cota: cotaConsorcio, req: ['valorCarta'] }
};
...
const F = {
  ...
  saude: `<div class="f"><label>Idade do titular</label><input id="idade" type="number" min="0" max="99" placeholder="34"></div>
   ...
   <div class="f full"><label>Cidade</label><input id="cidade" placeholder="Cotia, SP"></div>
  ...
  condominio: `<div class="f"><label>Nº de unidades</label><input id="unidades" type="number" min="2" max="500" placeholder="48"></div>
   ...
   <div class="f"><label>Valor em risco</label><select id="valorRisco">
  ...
  rc: `<div class="f"><label>Modalidade</label><select id="tipo">
   ...
   <div class="f full"><label>Atividade principal</label><input id="atividade" placeholder="Ex.: engenharia, clínica, evento"></div>`,
...
function timeline(prod) {
  const T = {
    ...
    saude: [['ATÉ 5 DIAS', 'Análise da operadora e confirmação da vigência.'], ['NA APROVAÇÃO', 'Primeira mensalidade e carteirinha digital.'], ['CARÊNCIAS', 'Urgência 24h · consultas 30d · internação 180d.']],
...
const box = document.getElementById('contratarBody');
const head = document.getElementById('contratarHead');
const $ = s => box.querySelector(s);
const LB = ['escolha o seguro', 'seus dados', 'ofertas disponíveis', 'contratação', 'pronto'];

function chrome() {
  document.querySelectorAll('#contratarSteps i').forEach((n, i) => n.classList.toggle('on', i <= Math.min(S.step, 3)));
  document.getElementById('contratarStepno').innerHTML = `<b>Passo 0${Math.min(S.step + 1, 4)}</b> · ${LB[S.step]}`;
}
...
function go(step, scroll = true) {
  ...
  chrome();
  render();
  if (scroll) document.getElementById('screens').scrollTop = 0;
}

function render() {
  [t0, t1, t2, t3, t4][S.step]();
}

function t0() {
  head.innerHTML = `<h2>Cote seu seguro</h2><p>Sem burocracia. Você vê o preço antes de informar qualquer documento.</p>`;
  box.innerHTML = `<div class="prods">${Object.entries(P).map(([k, p]) => `
    <button class="prod" data-p="${k}">
      <span class="ic">${svg(k, 20)}</span>
      <h3>${p.t}</h3>
      <span class="badge2 ${p.imediato ? 'now' : 'an'}">${p.tag}</span>
    </button>`).join('')}</div>
    <p class="help">Prefere ajuda? <a href="https://wa.me/5511966337576">Fale com a Vanessa.</a></p>`;
...
function t1() {
  ...
  box.querySelectorAll('.chips').forEach(g => {
    ...
    g.querySelectorAll('.chip').forEach(c => {
      c.setAttribute('aria-pressed', String(S.d[k] === c.dataset.v));
  ...
  box.querySelectorAll('input,select').forEach(i => {
    if (S.d[i.id] !== undefined && S.d[i.id] !== '') i.value = S.d[i.id]; else S.d[i.id] = i.value || '';
...
function ofertas() {
  ...
  box.innerHTML = `${S.ofertas.map((o, i) => `
    ...
    <p class="help">Ficou em dúvida? <a href="https://wa.me/5511966337576">A Vanessa te ajuda a escolher.</a></p>`;
  box.querySelectorAll('.of .btn').forEach(b => b.onclick = () => { S.pick = S.ofertas[+b.dataset.i]; go(3); });
  $('#refaz').onclick = () => go(1);
}

function t3() {
  const o = S.pick, cons = S.prod === 'consorcio'; const dd = decl(S.prod), dt = dd[0], itens = dd[1];
  ...
  <p class="blk">${dt}</p>
  <div class="dps">${itens.map(x => `<label><input type="checkbox" class="q"> <span>${x}</span></label>`).join('')}</div>
